...
void Field_VariableSizeDraw( field_t *edit, int x, int y, qboolean smallSize, qboolean showCursor ) {
	...
	i = drawLen - ( Q_PrintStrlen( str, (qboolean)MV_USE102COLOR, ntModDetected ) );
...
void CL_KeyEvent (int key, qboolean down, unsigned time) {
	...
	if ( cls.keyCatchers & KEYCATCH_CONSOLE ) {
		...
		if ( cgvm ) {
			qboolean ret = (qboolean)VM_Call(cgvm, CG_KEY_EVENT, Key_GetProtocolKey(MV_GetCurrentGameversion(), key), down);
