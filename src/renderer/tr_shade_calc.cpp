...
float WAVEVALUENEW(genFunc_t func, float base, float amplitude, float phase, float freq) {
	...
	case GF_TRIANGLE:
		if (index < 0.25) {
			...
			index = 1.0 - ((index - 0.25) * 4);
		} else if (index < 0.75) {
			index = -((index - 0.5) * 4);
		} else if (index <= 1.0) {
			index = -(1.0 - ((index - 0.75) * 4));
		}
...
void RB_CalcTurbulentTexCoords( const waveForm_t *wf, float *st )
{
	...
	for ( i = 0; i < tess.numVertexes; i++, st += 2 )
	{
		...
		st[0] = s + NewSinTable( ( tess.xyz[i][0] + tess.xyz[i][2] )* 0.0009765625 + now ) * wf->amplitude;
		st[1] = t + NewSinTable( tess.xyz[i][1] * 0.0009765625 + now ) * wf->amplitude;
	}
}
